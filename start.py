...
def should_open_buy_trade(df,idx):
    ...
    if temp_macd.size != 0 and temp_delta.size != 0 :
        ...
        if (df.iloc[idx - 2:idx]['BidClose'].mean() > max(df.iloc[idx - 2:idx]['senkou_a'].mean(),df.iloc[idx - 2:idx]['senkou_b'].mean()) and
            ...
            df.iloc[idx-27:idx]['rsi'][df['tenkan_avg']>df['kijun_avg']].mean() < 65):
                print('Position interesting')
                if ((df['BidHigh'] - df['BidLow'])[idx-7:idx].max()<2*(df['BidHigh'] - df['BidLow'])[idx-27*2:idx].mean() and 
...
def should_open_sell_trade(df,idx):
    ...
    if temp_macd.size != 0 and temp_delta.size != 0 :
        idx_last_macd=temp_macd.index[-1]
        idx_last_delta=temp_delta.index[-1]
        if (df.iloc[idx - 2:idx]['BidClose'].mean() < min(df.iloc[idx - 2:idx]['senkou_a'].mean(),df.iloc[idx - 2:idx]['senkou_b'].mean()) and
...
def should_close_buy_trade(df,idx,idx_open,dj):
    ...
    elif df.iloc[idx]['BidOpen'] > df.iloc[idx]['Bollinger_2'] and candle_m2<0.5 and \
...
def should_close_sell_trade(df,idx,idx_open,dj):
    ...
    if df.iloc[idx]['BidClose'] > min(df.iloc[idx]['senkou_a'],df.iloc[idx]['senkou_b']) \
        ...
        df.iloc[idx]['signal'] < df.iloc[idx]['macd']:
        result ='Kill for Bollinger 0'
    elif df.iloc[idx]['BidOpen'] < df.iloc[idx]['Bollinger_0'] and candle_m2>-0.5 and \
