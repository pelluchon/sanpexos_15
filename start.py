...
def open_trade(df, fx, tick, trading_settings_provider,dj,dfd1):
    ...
    if df.iloc[-2]['AskClose'] < min(df.iloc[-2]['senkou_a'],df.iloc[-2]['senkou_b']) \
        ...
        if (open_price- tp)/(sl - open_price)>=2 and (sl-open_price) > (max(df.iloc[-5:-2]['AskHigh']) - min(df.iloc[-5:-2]['AskLow'])):
            try:
    ...
    #BUY TENDANCE
        ...
        if (tp-open_price) / (open_price - sl)>=2 and (open_price - sl) > (max(df.iloc[-5:-2]['AskHigh']) - min(df.iloc[-5:-2]['AskLow'])):
            try:
                amount=set_amount(int(Dict['amount']), dj)
